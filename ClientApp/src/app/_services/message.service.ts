...
export class MessageService {
      ...
      if (group.connections.some(x => x.username === otherUsername)) {
        this.messageThread$.pipe(take(1)).subscribe({
          next: messages => {
            ...
            this.messageThreadSouce.next([...messages]);//change array of message a new one with existing messages
          }
        })
      }
    })
  ...
  async sendMessage(recipientUsername: string, content: string) {
    return this.hubConnection?.invoke('SendMessage', {recipientUsername, content}).catch(x => console.log(x))
  }
